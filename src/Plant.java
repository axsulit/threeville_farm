/**
 * The Plant class represents the plant that grows from a seed planted on a tile.
 */
public class Plant {
    /**
    ...
    /**
     * Initializes the attributes of the Plant class.
     *
    ...
    public void displayPlantStats(){
        System.out.println(" Seed Name: " + getSeedDetails().getName()+
    ...
    public void isToHarvest(Tile tile, Stats stats){
        if(tile.isOccupied() && tile.getTileType()!=TileType.HAS_ROCK){
            ...
            if(MyFarm.getDay() == this.dayToHarvest && this.timesWatered >= getSeedDetails().getWaterNeeded() && this.timesFertilized >= getSeedDetails().getFertilizerNeeded()){
                this.toHarvest = true;
                tile.setTileType(TileType.TO_HARVEST);
            }
            // If harvest day is reached and minimum requirements have not been met
                ...
                tile.setTileType(TileType.WITHERED);
    ...
    private void setActualProduceNum(boolean toHarvest) {
        // If ready for harvest, generate random number from range determined by expectedMinProduce and expectedMaxProduce from the Seed class
        if(toHarvest){
